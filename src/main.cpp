...
void setup()
{
  ...
  Button.attachClick(ProcessKeypress);
  // Button.attachDoubleClick(doubleClick);       not used
  // Button.attachLongPressStop(longPressStop);   not used
  Button.attachLongPressStart(ProcessLongPress);
  Button.setDebounceMs(50);
  Button.setClickMs(400);                   // single click time
  Button.setPressMs(800);                   // double click time
...
void loop()
{
  ...
  // If x milliseconds passed, then it's time to switch LED off for blink effect
  if (millis() - pps_blink_time > PPS_BLINK_INTERVAL) digitalWrite(PPS_LED, LOW); 

  Button.tick();  // check the button
}
...
{  
  ...
  {
    u8g2_1.setFont(u8g2_font_open_iconic_all_2x_t);
    u8g2_1.drawGlyph(0, 41, 79);
    u8g2_1.setFont(u8g2_font_logisoso16_tf); 
    u8g2_1.setCursor(18, 41);
    u8g2_1.print(abs(latitude), 4);
    u8g2_1.print(" S"); 
  }  
  else
  {
    u8g2_1.setFont(u8g2_font_open_iconic_all_2x_t);
    u8g2_1.drawGlyph(0, 41, 82);
    u8g2_1.setFont(u8g2_font_logisoso16_tf); 
    u8g2_1.setCursor(18, 41);
    u8g2_1.print(abs(latitude), 4);
    u8g2_1.print(" N"); 
  ...
  {
    u8g2_1.setFont(u8g2_font_open_iconic_all_2x_t);
    u8g2_1.drawGlyph(0, 64, 80);
    u8g2_1.setFont(u8g2_font_logisoso16_tf); 
    u8g2_1.setCursor(18, 64);
    u8g2_1.print(abs(longitude), 4);
    u8g2_1.print(" W"); 
  }  
  else
  {
    u8g2_1.setFont(u8g2_font_open_iconic_all_2x_t);
    u8g2_1.drawGlyph(0, 64, 81);
    u8g2_1.setFont(u8g2_font_logisoso16_tf); 
    u8g2_1.setCursor(18, 64);
    u8g2_1.print(abs(longitude), 4);
    u8g2_1.print(" E"); 
...
{
  u8g2_1.setFont(u8g2_font_logisoso16_tf);   
  u8g2_1.setCursor(0, 18);
  u8g2_1.print("GPS NTP Sync");
}  
...
void ShowClient()
{
  ...
  u8g2_2.drawGlyph(65, 18, 208);    // Server icon
  u8g2_2.drawGlyph(0, 64, 247);     // WiFi icon

  u8g2_2.setFont(u8g2_font_logisoso16_tf);

  u8g2_2.setCursor(0, 18);
  u8g2_2.print("Clients");

  u8g2_2.setCursor(85, 18);
...
{
  if (statusDisp1) statusDisp1 = 0;
  else statusDisp1 = 1;

  DEBUG_PRINTLN(F("LONG PRESS PROCESSED!"));
}
